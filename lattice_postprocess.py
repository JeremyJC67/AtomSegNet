#!/usr/bin/env python3
"""Apply lattice-like geometric constraints to AtomSegNet detections.

This script is designed to *augment* existing detection points with additional
points inferred from a lattice-like arrangement, rather than deleting them.
It works in per-image result folders such as:

  atomsegnet_udvd_results_full/1742_udvd_incnb_0000/
    1742_udvd_incnb_0000_pos_gaussian_blur_Gen1-gaussianMask.txt

For each folder it:
  - Loads the specified *_pos_*.txt file.
  - Optionally splits points by connected components in a mask.
  - In each group, fits a sinusoidal curve in a PCA-derived frame, identifies
    missing lattice sites (integer steps along the main axis), and adds new
    points at those sites while keeping all original detections.
  - Writes an augmented coordinate file with a configurable suffix.
  - Draws an overlay PNG on the provided base image (if available).
"""
...
def split_by_mask(points: np.ndarray, mask: np.ndarray | None) -> list[np.ndarray]:
    """Split points by connected components; keep points outside labels as one group."""
...
def fit_lattice_group(points: np.ndarray, resid_mult: float) -> np.ndarray:
    """Augment a group by filling missing lattice sites; keep original points."""
    if points.size < 10:
        return points
    ...
    # support points close to the curve; originals are always kept
    ...
    s0 = float(np.median(s_support))
    k_support = np.rint((s_support - s0) / delta).astype(np.int64)
    existing_k = sorted(set(int(kk) for kk in k_support))
    if len(existing_k) < 2:
        return points

    # prototype bbox and score from this group
    ...
    new_rows = []
    k_min, k_max = existing_k[0], existing_k[-1]
    for k in range(k_min, k_max + 1):
        if k in existing_k:
            continue
        # only fill internal gaps where both neighbours exist
        if (k - 1) not in existing_k or (k + 1) not in existing_k:
            continue
        s_k = s0 + k * delta
        d_k = A * np.sin(omega * s_k + phi) + c
        coord = mean[0] + s_k * u + d_k * v
    ...
    # drop any new points that are too close to existing ones
    coords_orig = np.column_stack([points["cx"], points["cy"]])
    coords_new = np.column_stack([new_pts["cx"], new_pts["cy"]])
    keep_new = np.ones(new_pts.shape[0], dtype=bool)
    for i, (x, y) in enumerate(coords_new):
        dist = np.hypot(coords_orig[:, 0] - x, coords_orig[:, 1] - y)
        if np.any(dist < 1.0):
            keep_new[i] = False
...
def apply_lattice(points: np.ndarray, mask: np.ndarray | None, resid_mult: float) -> np.ndarray:
    if points.size == 0:
        return points
    groups = split_by_mask(points, mask)
    augmented = [fit_lattice_group(g, resid_mult) for g in groups]
...
def process_directory(directory: Path, args: argparse.Namespace) -> Tuple[int, int]:
    ...
    augmented = apply_lattice(pts, mask, args.resid_mult)
    kept = augmented.size
    ...
    if args.dataset_root is not None and kept > 0:
        # Primary: look for <stem>.jpg under dataset_root
        base_img = args.dataset_root / f"{stem}.jpg"
        # Fallback for two-stage dirs like "1742_udvd_incnb_0000_denoise":
        # strip the "_denoise" suffix and try "<stem_no_suffix>.jpg"
        if not base_img.exists() and stem.endswith("_denoise"):
            stem2 = stem[: -len("_denoise")]
            candidate = args.dataset_root / f"{stem2}.jpg"
            if candidate.exists():
                base_img = candidate
        if base_img.exists():
    ...
    if args.verbose:
        print(f"{stem}: raw={raw} after_fill={kept}")
    return raw, kept


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Apply lattice-like geometric constraints to detections.")
    ...
    p.add_argument(
        ...
        type=float,
        default=1.0,
        help="Multiplier on base residual threshold (larger keeps more support points).",
    )
...
def main() -> None:
    ...
    for d in selection:
        ...
        total_raw += raw
        total_kept += kept
    print(f"Processed {len(selection)} folders; raw={total_raw}, after_fill={total_kept}.")
